...
wizardItems:any = [];
currentUserData = this.appGlobalService.getCurrentUserData();
selectedItems:any ={};
workflowType = "";
...
isSaving:boolean = false;
  ...
  "type" : "form"
	...
	constructor(public table1Service: Table1Service, public appUtilBaseService: AppUtilBaseService, public translateService: TranslateService, public messageService: MessageService, public confirmationService: ConfirmationService, public dialogService: DialogService, public domSanitizer: DomSanitizer, public bsModalService: BsModalService, public activatedRoute: ActivatedRoute, public breadcrumbService: BreadcrumbService, public appBaseService: AppBaseService, public router: Router, public appGlobalService: AppGlobalService, public uploaderService: UploaderService, public location: Location, ...args: any) {
    ...
    for (const property in this.formFieldConfig) {
      if ((this.formFieldConfig[property]?.uiType?.toLowerCase() == 'datetime' || this.formFieldConfig[property]?.uiType?.toLowerCase() == 'date') && (this.formFieldConfig[property]?.minDate || this.formFieldConfig[property]?.maxDate)) {
        if (this.formFieldConfig[property]?.minDate) {
          this.detailFormControls.controls[property].addValidators(dateValidator('min', this.formFieldConfig[property]))
        }
        if (this.formFieldConfig[property]?.maxDate) {
          this.detailFormControls.controls[property].addValidators(dateValidator('max', this.formFieldConfig[property]))
        }
      }
    }
  }
